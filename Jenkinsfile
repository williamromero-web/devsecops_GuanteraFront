        ...
        stage('🔑 Gitleaks Analysis') {
                    steps {
                        catchError(buildResult: 'SUCCESS', stageResult: 'UNSTABLE') {
                            ...
                            # Movemos tus herramientas fuera del alcance de Gitleaks
                            mv devsecops /tmp/devsecops_${BUILD_NUMBER} || true
                            mv Jenkinsfile /tmp/Jenkinsfile_${BUILD_NUMBER} || true

                            echo "⚙️ Ejecutando Gitleaks..."
                            # Gitleaks escanea un directorio limpio, pensando que tus scripts no existen
                            ./gitleaks detect --source="." --no-git \
                                --report-path="${REPORT_DIR}/security/gitleaks-report.json" \
                                --report-format=json || true

                            echo "🪄 Restaurando archivos..."
                            # Devolvemos todo a su lugar para que los siguientes stages puedan usarlos
        ...
        stage('📦 OWASP Dependency Check') {
            steps {
                catchError(buildResult: 'SUCCESS', stageResult: 'UNSTABLE') {
                    ...
                    echo "🚀 Iniciando OWASP SCA (Este proceso tardará la primera vez)..."
                    
                    # Creamos el volumen si no existe
                    docker volume create owasp_db_data || true
                    
                    # Ejecutamos con parámetros de optimización de red
        ...
        stage('🗂 Generación de Reportes PDF') {
            steps {
                catchError(buildResult: 'SUCCESS', stageResult: 'UNSTABLE') {
                    ...
                    echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
                    # ¡Añade Pillow al final de esta línea!
                    python3 -m pip install reportlab Pillow --quiet --break-system-packages

                    chmod +x devsecops/*.py
                    
                    # Asegurar la librería de PDF
                    python3 -m pip install reportlab --quiet --break-system-packages
