...
function getExtendedPalette(
  p: Theme["palette"],
): Theme["palette"] & ExtendedPaletteFields {
...
function getTextFallbacks(p: Theme["palette"], isDark: boolean) {
  ...
  return {
    ...
    tertiary:
      (p.text as { tertiary?: string })?.tertiary ??
      (isDark ? "#546E7A" : "#BDBDBD"),
    menuActive: (p.text as { menuActive?: string })?.menuActive ?? textPrimary,
    menuDefault:
      (p.text as { menuDefault?: string })?.menuDefault ?? textSecondary,
  };
}

function getCardFallbacks(p: Theme["palette"], isDark: boolean) {
  const paper = p.background?.paper ?? (isDark ? "#000000" : "#FFFFFF");
  const textSecondary = p.text?.secondary ?? (isDark ? "#FFFFFF" : "#696969");
  const textPrimary = p.text?.primary ?? (isDark ? "#EBFCF8" : "#181818");
  return {
    background: paper,
    border: isDark ? "#1F1F1F" : "#E0E0E0",
    label: textSecondary,
    plate: textPrimary,
    value: textPrimary,
    online: isDark
      ? "#1B5E20"
      : ((p.success as { light?: string })?.light ?? "#E8F5E9"),
    offline: isDark
      ? "#B71C1C"
      : ((p.error as { light?: string })?.light ?? "#FFEBEE"),
    warning: isDark
      ? "#E65100"
      : ((p.warning as { light?: string })?.light ?? "#FFF3E0"),
...
function getFallbackPalette(p: Theme["palette"], isDark: boolean) {
  ...
  return {
    surface: {
      main: pa.surface?.main ?? (isDark ? "#000000" : "#EBFCF8"),
      alt: pa.surface?.alt ?? (isDark ? "#000000" : "#E7FFFA"),
    },
    border: {
      main: pa.border?.main ?? (isDark ? "#37474F" : "#E0E0E0"),
    },
    alwaysDark: {
      main: pa.alwaysDark?.main ?? (isDark ? "#000000" : "#090907"),
    },
    text: getTextFallbacks(p, isDark),
    scrollbar:
      pa.scrollbar ??
      (isDark
        ? { thumb: "#003833", track: "#000000" }
        : { thumb: "#00F1C7", track: "#F6F6F6" }),
